...
def Qlearn():
    ...
    while episode < (MAX_EPISODE/3):
        ...
        while not end and not rospy.is_shutdown() and steps <50:
            ...
            if (state==11):
                total_action_taken2 +=1
                if act == 'FORWARD_AND_TURN_LITTLE_RIGHT':
                    reward+=10
                else:
                    reward-=10
            
            if ((state==19 or state==20 or state ==22 or state ==23 or state ==24 or state==25)):
                if act == 'FORWARD_AND_HARD_LEFT':
                    reward-=20
                if act == 'FORWARD_AND_TURN_LITTLE_LEFT':
                    reward-=10
        ...
        totalTrainingRewards.append(total_training_rewards)
        plt.clf()
        plt.plot(episode_numbers, totalTrainingRewards, label='Training Rewards')
...
# LOST_DISTANCE_THRESHOLD =0.7    
    ...
    if (min_right <0.05):
        right ="vclose"  
    elif ( 0.05<min_right< 0.35):
        right= "close"
    elif (0.35 <min_right < 0.4):
        right = "normal0"
    #The below was from 0.4 to 0.55
